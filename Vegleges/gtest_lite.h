...
#define EXPECT_STREQ(expected, actual) ((actual != NULL) ? EXPECTCMP(strcmp(expected, actual) == 0, expected, actual) : EXPECT(false, "STR_EQ NULL pointert kapott!"))

/// C stringek (const char *) eltéréset tesztelő makró
#define EXPECT_STRNE(expected, actual) ((actual != NULL) ? EXPECTNE(strcmp(expected, actual) != 0, expected, actual) : EXPECT(false, "STR_EQ NULL pointert kapott!"))
